...
impl Paseto {
    ...
    pub fn validate_access_token<T>(&self, token: &str) -> Result<T>
        where T: CustomClaims
    {
        ...
        let claims = T::convert_from(result);

        // Return claims
        Ok(claims)
    }

    pub fn validate_refresh_token<T>(&self, token: &str) -> Result<T>
        where T: CustomClaims
    {
