...
impl S3 {
        ...
        where T: ToString
    {
        ...
        let extension = parsers::ext_from_mime(mime.clone());
        ...
        if Asset::is_image(&mime) {
            ...
            let image = match image::load_from_memory(&data) {
                Ok(image) => image,
                Err(error) => return Err(Errors::to(
                    Response::InternalServerError,
                    error.to_string()
                ))
            };
        ...
        // Set file container
        let file = Asset{
            filename: Some(parsers::change_ext(legacy_filename.clone(), "")),
        ...
        where T: ToString
    {
        ...
        let filename = parsers::change_ext(filename, "webp");
