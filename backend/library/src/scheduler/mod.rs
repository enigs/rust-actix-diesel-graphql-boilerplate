...
impl Actor for Scheduler {
    ...
    fn started(&mut self, ctx: &mut Context<Self>) {
        println!("=== Cron started running at {} ===", Local::now().format("%b %d, %Y - %I:%M%p").to_string());
    ...
    fn stopped(&mut self, _ctx: &mut Context<Self>) {
        println!("=== Cron stopped running at {} ===", Local::now().format("%b %d, %Y - %I:%M%p").to_string());
    }
}
...
impl Scheduler {
    ...
    fn schedule_task(&self, ctx: &mut Context<Self>) {
        ...
        println!("Task {}: ", Local::now().format("%b %d, %Y - %I:%M%p").to_string());
